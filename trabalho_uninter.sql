------------------------------------
-- CÓDIGO USADO PARA O TRABALHO.
-- Carlo Benetti
-- 03/05/2025
------------------------------------



SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';


CREATE SCHEMA IF NOT EXISTS `TrabalhoUninter` ;
USE `TrabalhoUninter` ;
show databases;

-- -----------------------------------------------------
-- Tabela Cliente
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TrabalhoUninter`.`Cliente` (
  `idCliente` INT NOT NULL,
  `CPF` VARCHAR(20) NOT NULL,
  `Nome` VARCHAR(50) NOT NULL,
  `telefone` VARCHAR(20) NOT NULL,
  `email` VARCHAR(50) NOT NULL,
  `endereço` VARCHAR(100) NOT NULL,
  PRIMARY KEY (`idCliente`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Tabela Pagamento
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TrabalhoUninter`.`Pagamento` (
  `idPagamento` INT NOT NULL,
  `forma_pagamento` ENUM('Cartão', 'PIX', 'Dinheiro') NOT NULL,
  `dataPagamento` DATE NOT NULL,
  `valorTotal` DECIMAL(7,2) NOT NULL,
  `estado` ENUM('Pago', 'Pendente') NOT NULL,
  PRIMARY KEY (`idPagamento`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Tabela Locação
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TrabalhoUninter`.`Locacao` (
  `idLocacao` INT NOT NULL,
  `dataInicio` DATE NOT NULL,
  `dataFim` DATE NOT NULL,
  `Cliente_idCliente` INT NOT NULL,
  `Pagamento_idPagamento` INT NOT NULL,
  PRIMARY KEY (`idLocacao`, `Cliente_idCliente`, `Pagamento_idPagamento`),
  INDEX `fk_Locacao_Cliente_idx` (`Cliente_idCliente` ASC) VISIBLE,
  INDEX `fk_Locacao_Pagamento1_idx` (`Pagamento_idPagamento` ASC) VISIBLE,
  CONSTRAINT `fk_Locacao_Cliente`
    FOREIGN KEY (`Cliente_idCliente`)
    REFERENCES `TrabalhoUninter`.`Cliente` (`idCliente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Locacao_Pagamento1`
    FOREIGN KEY (`Pagamento_idPagamento`)
    REFERENCES `TrabalhoUninter`.`Pagamento` (`idPagamento`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

-- -----------------------------------------------------
-- Tabela veículo
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TrabalhoUninter`.`Veiculo` (
  `idVeiculo` INT NOT NULL,
  `modelo` VARCHAR(50) NOT NULL,
  `marca` VARCHAR(50) NOT NULL,
  `ano` INT NOT NULL,
  `placa` VARCHAR(10) NOT NULL,
  `valorDiario` DECIMAL(7,2) NOT NULL,
  `estado` ENUM('Disponível', 'Alugado', 'Manutenção') NOT NULL,
  PRIMARY KEY (`idVeiculo`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Tabela relação Locação Veículo
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TrabalhoUninter`.`LocacaoVeiculo` (
  `Veiculo_idVeiculo` INT NOT NULL,
  `Locacao_idLocacao` INT NOT NULL,
  `Locacao_Cliente_idCliente` INT NOT NULL,
  `Locacao_Pagamento_idPagamento` INT NOT NULL,
  PRIMARY KEY (`Veiculo_idVeiculo`, `Locacao_idLocacao`, `Locacao_Cliente_idCliente`, `Locacao_Pagamento_idPagamento`),
  INDEX `fk_LocacaoVeiculo_Locacao1_idx` (`Locacao_idLocacao` ASC, `Locacao_Cliente_idCliente` ASC, `Locacao_Pagamento_idPagamento` ASC) VISIBLE,
  CONSTRAINT `fk_LocacaoVeiculo_Veiculo1`
    FOREIGN KEY (`Veiculo_idVeiculo`)
    REFERENCES `TrabalhoUninter`.`Veiculo` (`idVeiculo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_LocacaoVeiculo_Locacao1`
    FOREIGN KEY (`Locacao_idLocacao` , `Locacao_Cliente_idCliente` , `Locacao_Pagamento_idPagamento`)
    REFERENCES `TrabalhoUninter`.`Locacao` (`idLocacao` , `Cliente_idCliente` , `Pagamento_idPagamento`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Tabela Manuntenção
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TrabalhoUninter`.`Manutencao` (
  `idManutencao` INT NOT NULL,
  `descricao` VARCHAR(100) NOT NULL,
  `dataManutencao` DATE NOT NULL,
  `custo` DECIMAL(7,2) NOT NULL,
  `Veiculo_idVeiculo` INT NOT NULL,
  PRIMARY KEY (`idManutencao`, `Veiculo_idVeiculo`),
  INDEX `fk_Manutencao_Veiculo1_idx` (`Veiculo_idVeiculo` ASC) VISIBLE,
  CONSTRAINT `fk_Manutencao_Veiculo1`
    FOREIGN KEY (`Veiculo_idVeiculo`)
    REFERENCES `TrabalhoUninter`.`Veiculo` (`idVeiculo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;




-- Script de inserção

USE TrabalhoUninter;

-- Inserção de Clientes
INSERT INTO Cliente (idCliente, CPF, Nome, telefone, email, endereço) VALUES
  (1, '045.159.785-10', 'João da Silva', '(41) 99854-5648', 'joao.s@email.com.br', 'Rua das Flores, 3578, Curitiba - PR'),
  (2, '147.963.789-02', 'Maria de Souza', '(21) 99763-1213', 'maria.s@email.com.br', 'Rua Vicente Machado, 10503, apto. 1007, Rio de Janeiro - RJ'),
  (3, '357.456.309-07', 'Carlos Mendes', '(85) 98754-2050', 'carlos.m@email.com.br', 'Rua Osvaldo Cruz, 578, Fortaleza - CE'),
  (4, '569.428.308.12', 'Ana Oliveira', '(11) 99527-3721', 'ana.o@email.com.br', 'Avenida Ipiranga, 1246, apto. 201, São Paulo - SP'),
  (5, '096.429.761-05', 'Pedro dos Santos', '(92) 98418-3141', 'pedro.s@email.com.br', 'Rua Venezuela, 649, Manaus - AM'),
  (6, '548.302.980-13', 'Fernanda Lima', '(51) 99234-5458', 'fernanda.l@email.com.br', 'Avenida Uruguai, 3152, apto. 2202, Porto Alegre - RS'),
  (7, '947.652.415-09', 'Ricardo Alves', '(65) 98953-7828', 'ricardo.a@email.com.br', 'Rua dos Açudes, 1029, Cuiabá - MT'),
  (8, '012.359.678-15', 'Juliana Costa', '(63) 99315-6264', 'juliana.c@email.com.br', 'Avenida Brasil, 953, apto. 709, Palmas - TO'),
  (9, '254.635.485-05', 'Lucas Martins', '(71) 99264-3585', 'lucas.m@email.com.br', 'Rua dos Baianos, 12549, Salvador - BA'),
  (10, '785.126.497-72', 'Beatriz Rocha', '(69) 98767-1545', 'beatriz.r@email.com.br', 'Rua Indenpendente, 209, Porto Velho - RO');

-- Inserção de Veículos
INSERT INTO Veiculo (idVeiculo, modelo, marca, ano, placa, valorDiario, estado) VALUES
  (1, 'Onix', 'Chevrolet', 2021, 'ABC-1D34', 150.00, 'Disponível'),
  (2, 'HB20', 'Hyundai', 2022, 'XYZ-5E78', 180.00, 'Alugado'),
  (3, 'Gol', 'Volkswagen', 2020, 'DEF-9W12', 140.00, 'Alugado'),
  (4, 'Civic', 'Honda', 2023, 'GHI-3F56', 250.00, 'Disponível'),
  (5, 'Corolla', 'Toyota', 2022, 'JKL-7I90', 240.00, 'Alugado'),
  (6, 'Fiesta', 'Ford', 2019, 'MNO-2P45', 130.00, 'Manutenção'),
  (7, 'Cruze', 'Chevrolet', 2021, 'PQR-6A89', 200.00, 'Disponível'),
  (8, 'Duster', 'Renault', 2022, 'STU-1Z34', 220.00, 'Alugado'),
  (9, 'Toro', 'Fiat', 2023, 'VWX-5Q78', 260.00, 'Manutenção'),
  (10, 'Compass', 'Jeep', 2024, 'YZA-9S12', 280.00, 'Disponível');

-- Inserção de Manutenções
INSERT INTO Manutencao (idManutencao, Veiculo_idVeiculo, descricao, dataManutencao, custo) VALUES
  (1, 1, 'Troca de óleo e revisão geral', '2024-12-09', 200.00),
  (2, 1, 'Substituição de pneu', '2024-12-10', 600.00),
  (3, 3, 'Troca de pastilhas de freio', '2024-12-14', 450.00),
  (4, 5, 'Alinhamento e balanceamento', '2024-12-18', 150.00),
  (5, 5, 'Revisão elétrica completa', '2024-12-28', 500.00),
  (6, 8, 'Reparo na suspensão', '2025-01-05', 700.00),
  (7, 10, 'Troca do sistema de escapamento', '2025-01-07', 750.00),
  (8, 6, 'Troca de bateria', '2025-01-17', 400.00),
  (9, 6, 'Substituição do filtro de ar', '2025-01-17', 120.00),
  (10, 9, 'Pintura e retoques na lataria', '2025-01-28', 900.00);

-- Inserção de Pagamentos
INSERT INTO Pagamento (idPagamento, forma_pagamento, dataPagamento, valorTotal, estado) VALUES
  (1, 'Cartão', '2024-12-01', 360.00, 'Pago'),
  (2, 'Cartão', '2024-12-01', 140.00, 'Pago'),
  (3, 'Pix', '2024-12-05', 480.00, 'Pago'),
  (4, 'Cartão', '2024-12-10', 1100.00, 'Pago'),
  (5, 'Dinheiro', '2024-12-11', 1350.00, 'Pago'),
  (6, 'Cartão', '2024-12-15', 2210.00, 'Pago'),
  (7, 'Pix', '2024-12-20', 1080.00, 'Pago'),
  (8, 'Pix', '2024-12-23', 2200.00, 'Pago'),
  (9, 'Cartão', '2024-12-26', 500.00, 'Pago'),
  (10, 'Pix', '2024-12-28', 840.00, 'Pago'),
  (11, 'Pix', '2025-01-02', 520.00, 'Pago'),
  (12, 'Dinheiro', '2025-01-05', 900.00, 'Pago'),
  (13, 'Cartão', '2025-01-05', 260.00, 'Pago'),
  (14, 'Cartão', '2025-01-09', 1120.00, 'Pago'),
  (15, 'Pix', '2025-01-12', 600.00, 'Pago'),
  (16, 'Cartão', '2025-01-13', 1680.00, 'Pendente'),
  (17, 'Pix', '2025-01-16', 1040.00, 'Pago'),
  (18, 'Pix', '2025-01-18', 540.00, 'Pendente'),
  (19, 'Cartão', '2025-01-19', 280.00, 'Pendente'),
  (20, 'Pix', '2025-01-21', 880.00, 'Pendente');

-- Inserção de Locações
INSERT INTO Locacao (idLocacao, dataInicio, dataFim, Cliente_idCliente, Pagamento_idPagamento) VALUES
  (1, '2024-12-01', '2024-12-03', 1, 1),
  (2, '2024-12-01', '2024-12-02', 2, 2),
  (3, '2024-12-05', '2024-12-07', 3, 3),
  (4, '2024-12-10', '2024-12-15', 1, 4),
  (5, '2024-12-11', '2024-12-20', 4, 5),
  (6, '2024-12-15', '2025-01-01', 2, 6),
  (7, '2024-12-20', '2024-12-26', 5, 7),
  (8, '2024-12-23', '2025-01-02', 6, 8),
  (9, '2024-12-26', '2024-12-28', 1, 9),
  (10, '2024-12-28', '2025-01-03', 3, 10),
  (11, '2025-01-02', '2025-01-04', 7, 11),
  (12, '2025-01-05', '2025-01-10', 4, 12),
  (13, '2025-01-05', '2025-01-07', 6, 13),
  (14, '2025-01-09', '2025-01-13', 8, 14),
  (15, '2025-01-12', '2025-01-15', 7, 15),
  (16, '2025-01-13', '2025-01-20', 9, 16),
  (17, '2025-01-16', '2025-01-20', 10, 17),
  (18, '2025-01-18', '2025-01-21', 9, 18),
  (19, '2025-01-19', '2025-01-20', 5, 19),
  (20, '2025-01-21', '2025-01-25', 1, 20);

-- Inserção de LocaçãoVeículo
INSERT INTO LocacaoVeiculo (Veiculo_idVeiculo, Locacao_idLocacao, Locacao_Cliente_idCliente, Locacao_Pagamento_idPagamento) VALUES
  (2, 1, 1, 1),
  (3, 2, 2, 2),
  (5, 3, 3, 3),
  (8, 4, 1, 4),
  (1, 5, 4, 5),
  (6, 6, 2, 6),
  (2, 7, 5, 7),
  (8, 8, 6, 8),
  (4, 9, 1, 9),
  (3, 10, 3, 10),
  (9, 11, 7, 11),
  (2, 12, 4, 12),
  (6, 13, 6, 13),
  (10, 14, 8, 14),
  (7, 15, 7, 15),
  (5, 16, 9, 16),
  (9, 17, 10, 17),
  (2, 18, 9, 18),
  (10, 19, 5, 19),
  (8, 20, 1, 20);
  
  use Trabalhouninter;
  select * from LocacaoVeiculo;

